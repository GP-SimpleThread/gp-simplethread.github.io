class SimpleFirebase {
	...
	static get SimpleFirestore () {
		return class SimpleFirestore {
			constructor (simpleFbObj) {
				const self = Object.create(simpleFbObj.fbHandler.firestore(), {
					...
					getValue: {
						 ...
						 * 
						 * @returns {Promise<Array<firebase.firestore.QueryDocumentSnapshot> | Object> | Function}
						 */
						value (path, callback) {
							if (!callback) {
								return this.get(path).then(snapshot => snapshot.id ? snapshot.data() : snapshot.docs);
							} else {
								return this.ref(path).onSnapshot(snapshot => snapshot.id ? callback(snapshot.data()) : callback(snapshot.docs));
							}
						}
					},
			 ...
			 * @param {Function<firebase.firestore.DocumentSnapshot, Number, Array<firebase.firestore.DocumentSnapshot>>} callback
			 * 
			 * @returns {Array<firebase.firestore.DocumentSnapshot>}
			 */
			static filter (snapshot, callback) {
				return Array.prototype.filter.call(snapshot.docs, callback);
			}
		};
	}
}
...
class SimpleFirebaseUI {
	...
	constructor (simpleFbObj, parentSelector = "Body") {
		this.config = { signInOptions: [], callbacks: {} };
		this.selector = parentSelector;
		
		this.fbUiHandler = new firebaseui.auth.AuthUI(simpleFbObj.Auth);
	}
