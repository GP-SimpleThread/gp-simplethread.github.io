class Locale {
	static load (languageCode = "en") {
		return fetch(`${Linker.rootDir}/locales/${languageCode}.json`).catch(error => fetch(`${Linker.rootDir}/locales/en.json`)).then(response => response.json()).then(messages => messages);
	}
...
(() => {
	...
	if (querys.has("lang")) {
		...
		cookieStore.set("GP-ST-lang", querys.get("lang"), { path: "/" });
